...
//Foliage
Texture2D gFoliageMap;
float3 gFoliageSwayDir;
float gFoliageSwaySpeed;
float gFoliageSwayStrength;
float gFoliagePosMult;
...
float3 CalculateNormal(float3 tangent, float3 binormal, float3 normal, float2 texCoord)
{
    ...
    if(gUseNormalMap)
    {
        ...
        float3 sampledNormal = gNormalMap.Sample(samLinear, texCoord).xyz;
...
VS_OUTPUT VS(VS_INPUT input)
{
    ...
    //Get dimensions
    float w, h;
    gFoliageMap.GetDimensions(w, h);
    uint2 uv = abs(uint2(uint(input.texCoord.x * w) % w, uint(input.texCoord.y * h) % h));
    
    //Modify pos according to sway
    float3 swayPos = input.pos;
    float intensity = gFoliageMap.Load(int3(uv, 0));
    swayPos += normalize(mul(gFoliageSwayDir, (float3x3) gWorld)) * sin(swayPos * gFoliagePosMult + gTotalTime * gFoliageSwaySpeed) * gFoliageSwayStrength * intensity;
    //The sway dir is transformed according to the world rotation too, to keep consistency
    
    //Transform
    output.pos = mul(float4(swayPos, 1.f), gWorldViewProj); //Final pos
    output.worldPos = mul(float4(swayPos, 1.f), gWorld);
    ...
    //Light pos
    output.lPos = mul(float4(swayPos, 1.f), gWorldViewProj_Light);
...
float4 PS(VS_OUTPUT input) : SV_TARGET
{
    ...
    float3 lightWarp = gLightWarpMap.Sample(samWarp, float2(diffuseStrength, diffuseStrength)).xyz;
