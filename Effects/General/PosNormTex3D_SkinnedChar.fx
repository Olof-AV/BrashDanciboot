...
float4 PS(VS_OUTPUT input) : SV_TARGET
{
	...
	//Fresnel
    float fresnel = pow(((1.f - saturate(abs(dot(input.normal, viewDirection))))), gFresnelPower) * gFresnelMult;
    float fresnelMask = pow(1.f - saturate(dot(float3(0.f, -1.f, 0.f), input.normal)), gFresnelHardness);
    float3 rimTerm = fresnel * fresnelMask * gFresnelColour;
	...
	//Obtain final colour
    color_rgb = saturate(diffuse * color_rgb + rimTerm + phong * shadowValue);
